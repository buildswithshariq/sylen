...
describe("carbonCalculator", () => {
  ...
  describe("calculateEnergyEmissions", () => {
    it("calculates base energy and AC correctly without appliance multiplier", () => {
      const data = {
        ...
        acHoursPerDay: 0,
        applianceUsage: "medium" as const,
      };
  ...
  describe("calculateFoodEmissions", () => {
    ...
    it("uses fallback for unknown diet type and food waste", () => {
      const data = {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        dietType: "unknown" as any,
        meatMealsPerWeek: 0,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        foodWaste: "unknown" as any,
      };
  ...
  describe("calculateLifestyleEmissions", () => {
    it("calculates flights and recycling", () => {
      const data = {
        flightsPerYear: 2,
        shoppingFrequency: "rarely" as const,
        recyclingHabit: "always" as const,
      };
      const expected =
        2 * FLIGHT_EMISSIONS_PER_TRIP + 200 + RECYCLING_OFFSETS.always;
    ...
    it("uses fallback for unknown shopping frequency and recycling habit", () => {
      const data = {
        flightsPerYear: 0,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        shoppingFrequency: "unknown" as any,
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        recyclingHabit: "unknown" as any,
      };
