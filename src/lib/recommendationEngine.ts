...
function getTransportRecommendations(data: AssessmentData): Recommendation[] {
  ...
  if (vehicleType === 'car') {
    ...
    recs.push({
      ...
      description: 'Sharing rides cuts your per-person transport emissions by up to 50% on carpool days.',
      category: 'transport',
    ...
    if (dailyDistanceKm <= 10) {
      recs.push({
        ...
        description: 'Your commute is short enough to bike or walk — zero emissions and great for health!',
        category: 'transport',
  ...
  if (vehicleType === 'public_transit' && dailyDistanceKm > 5) {
    recs.push({
      ...
      category: 'transport',
      estimatedReductionKg: Math.round(dailyDistanceKm * 0.089 * 365 * (1.5 / 5)),
      difficulty: 'easy',
      priority: 2,
      icon: '🏠',
    });
...
function getEnergyRecommendations(data: AssessmentData): Recommendation[] {
  ...
  if (acHoursPerDay > 6) {
    ...
    recs.push({
      ...
      description: 'Every degree higher saves approximately 6% of AC energy consumption.',
      category: 'energy',
    ...
    recs.push({
      ...
      description: 'Fans use 90% less energy than AC. Use them when the temperature is bearable.',
      category: 'energy',
  ...
  if (applianceUsage === 'high') {
    recs.push({
      ...
      category: 'energy',
      estimatedReductionKg: Math.round(monthlyElectricityKwh * 0.15 * 12 * 0.42),
      difficulty: 'hard',
      priority: 3,
      icon: '🔌',
    });
  ...
  if (monthlyElectricityKwh > 200) {
    recs.push({
      ...
      description: 'Standby power ("vampire energy") accounts for 5-10% of home energy use.',
      category: 'energy',
      estimatedReductionKg: Math.round(monthlyElectricityKwh * 0.07 * 12 * 0.42),
      difficulty: 'easy',
      priority: 4,
      icon: '🔋',
    });
...
function getFoodRecommendations(data: AssessmentData): Recommendation[] {
  ...
  if (dietType === 'heavy_meat' || dietType === 'mixed') {
    if (meatMealsPerWeek > 5) {
      recs.push({
        ...
        description: 'Replacing 2 days of meat with plant-based meals can significantly reduce your food emissions.',
        category: 'food',
    ...
    if (dietType === 'heavy_meat') {
      recs.push({
        ...
        description: 'Beef produces 5-10× more emissions than chicken. Swapping reduces impact dramatically.',
        category: 'food',
  ...
  if (foodWaste === 'significant') {
    recs.push({
      ...
      description: 'Plan meals weekly and buy only what you need. Food waste accounts for 8-10% of global emissions.',
      category: 'food',
    ...
    recs.push({
      ...
      description: 'Composting diverts waste from landfills where it would produce methane.',
      category: 'food',
  ...
  if (dietType === 'mixed' || dietType === 'vegetarian') {
    recs.push({
      ...
      description: 'Locally sourced food requires less transportation and refrigeration.',
      category: 'food',
...
function getLifestyleRecommendations(data: AssessmentData): Recommendation[] {
  ...
  if (flightsPerYear > 2) {
    recs.push({
      ...
      description: 'A single round-trip flight produces about 1.1 tonnes of CO₂e. Fewer flights = major impact.',
      category: 'lifestyle',
  ...
  if (shoppingFrequency === 'weekly') {
    recs.push({
      ...
      description: 'Reducing shopping frequency and choosing quality over quantity cuts manufacturing emissions.',
      category: 'lifestyle',
  ...
  if (recyclingHabit === 'never' || recyclingHabit === 'sometimes') {
    recs.push({
      ...
      description: 'Recycling reduces the need for raw material extraction and manufacturing.',
      category: 'lifestyle',
