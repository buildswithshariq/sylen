...
export default function MaintainImpactCard({ assessmentData, className = '' }: MaintainImpactCardProps) {
  const habits = [];

  if (assessmentData.transport.vehicleType === 'bicycle' || assessmentData.transport.vehicleType === 'walk' || assessmentData.transport.vehicleType === 'public_transit') {
  ...
  if (assessmentData.food.dietType === 'vegan' || assessmentData.food.dietType === 'vegetarian') {
  ...
  return (
    <GlassCard className={`p-6 sm:p-8 flex flex-col justify-between h-full ${className}`}>
      <div>
        <h3 className="text-lg font-bold text-emerald-800 mb-2">Maintain Your Impact</h3>
        <p className="text-sm text-stone-500 mb-6">
          You are already practicing these highly sustainable habits. Keep it up!
        </p>
        ...
        <ul className="space-y-3 mb-8">
          {habits.map((habit, index) => (
            ...
            >
              ...
              <span className="text-sm font-medium text-stone-700">{habit}</span>
            </motion.li>
          ))}
        </ul>
      </div>
      ...
      <div className="bg-emerald-50/50 rounded-xl p-5 border border-emerald-100/50">
        ...
        <p className="text-xs text-stone-600 leading-relaxed">
          The biggest impact you can have now is helping others. Share your sustainable practices with friends and family, or advocate for climate policies in your community.
        </p>
      </div>
    </GlassCard>
  );
