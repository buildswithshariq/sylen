...
export default function CommunityImpactWidget({
  ...
  return (
    <GlassCard className={`p-6 flex flex-col justify-between ${className}`}>
      <div>
        <h3 className="text-lg font-semibold text-stone-800 mb-2">Community Impact</h3>
        {isBetter ? (
          <p className="text-stone-600 text-sm mb-4">
            You emit <span className="font-bold text-emerald-600">{difference.toLocaleString()} kg</span> less than the average person annually.
          </p>
        ) : (
          <p className="text-stone-600 text-sm mb-4">
            You emit <span className="font-bold text-rose-500">{Math.abs(difference).toLocaleString()} kg</span> more than average. Small changes can flip this!
          </p>
        )}
      </div>
        ...
        <div className="bg-white/50 rounded-xl p-4 border border-white/60">
          {isBetter ? (
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-stone-600">It would be equivalent to planting</p>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-rose-600">
                  {Math.round(Math.abs(difference) * 1000 / 20).toLocaleString()} extra trees
                </p>
              </div>
            </div>
          )}
