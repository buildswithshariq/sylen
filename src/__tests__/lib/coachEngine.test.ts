...
const mockContext: CoachContext = {
  score: {
    ...
    categoryLabel: "Great",
    category: "good",
    breakdown: { transport: 4000, energy: 3000, food: 2000, lifestyle: 1000, total: 10000 },
    contributions: [
      {
        category: "transport",
        label: "Transport",
...
describe("coachEngine", () => {
  describe("buildSystemPrompt", () => {
    ...
    it("generates champion prompt when score >= 90", () => {
      const championContext = {
        ...mockContext,
        score: { ...mockContext.score, score: 95 },
      };
    ...
    it("handles comparisonToAverage > 0", () => {
      const highEmissionsContext = {
        ...mockContext,
        score: { ...mockContext.score, comparisonToAverage: 25 },
      };
    ...
    it("handles ecoLevel.next being null", () => {
      const maxLevelContext = {
        ...mockContext,
        ecoLevel: {
          current: mockContext.ecoLevel.current,
          pointsToNext: 0,
          next: null,
          progressPercent: 100,
        },
      ...
      const transitContext = {
        ...
        assessment: {
          ...
          transport: {
            ...
            vehicleType: "public_transit" as any,
          },
        },
      };
  ...
  describe("buildChatMessages", () => {
    ...
    it("maps non-user roles to model", () => {
      const history = [{ id: "1", role: "assistant" as const, content: "Hello", timestamp: Date.now() }];
  ...
  describe("generateInitialGreeting", () => {
    ...
    it("returns standard personalized greeting", () => {
      expect(generateInitialGreeting(mockContext)).toContain(
        "Welcome back to Sylen",
      );
    });
    ...
    it("returns champion greeting", () => {
      ...
      expect(generateInitialGreeting(championContext)).toContain(
        "🏆 Congratulations",
      );
    });
  });
  ...
  describe("getFallbackResponse", () => {
    ...
    it("handles vegan diet", () => {
      ...
      expectResponseToContain("food", context, "plant-based diet");
    });
      ...
      const context = {
        ...
        assessment: {
          ...
          food: {
            ...mockContext.assessment.food,
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            dietType: "heavy_meat" as any,
          },
        },
      };

      expectResponseToContain("food", context, "meat-free day");
    });

    it("handles generic queries", () => {
      expectResponseToContain("hello", null, "Hi there! I'm Sprout");
    });
